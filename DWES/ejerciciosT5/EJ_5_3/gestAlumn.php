        ...
        try {
            if(empty($votoName)){
                ...
                if($user === $votoName){
                    ...
                    try {
                        $sql = 'SELECT user FROM alumnos WHERE user=:votoNombre';
                        $stmt = $conn->prepare($sql);
                        $stmt->bindParam(':votoNombre', $votoName);
                        $stmt->execute();
                        ...
                        if ($stmt->rowCount()>0) {
                            ...
                            throw new Exception("El nombre de quien deseas votar no está en la BBDD.");
                        }
                    } catch (PDOException $e) {
                        echo $e->getMessage();
                        header('Refresh:3, url= alumno.php');
